...
Hooks.once('ready', () => {
    console.log("Death's Door | Modul geladen. Nahtloser Stabilisierungs-Abbruch aktiv.");
...
Hooks.on('updateActor', (actor, changes, options, userId) => {
    ...
    // --- LOGIK: STABILISIERUNG AUFHEBEN (OHNE HP-TRICK) ---
    // 1. Wenn der Spieler HP bekommt, wacht er auf.
    if (currentHp > 0) {
        isStabilized = false;
    }

    // 2. WENN DIE FEHLSCHLÄGE STEIGEN -> SOFORT WIEDER STERBEND!
    if (deathFailures > prevState.failure) {
        isStabilized = false;
    }

    // --- LOGIK: STABILISIERUNG AKTIVIEREN ---
    // 3. Durch "Stabilisieren" Button des GMs (Setzt Erfolge & Fehlschläge auf 0)
    ...
    // 4. Durch 3 erwürfelte Erfolge (Wird nur getriggert in der Sekunde, wo der 3. Erfolg erreicht wird)
    if (deathSuccesses >= 3 && prevState.success < 3) {
        isStabilized = true;
    }
    ...
    // Treffer bei 0 HP für das Kamera-Beben
    const tookDamageWhileDown = isDying && (deathFailures > prevState.failure);

    // Zustand für den nächsten Check speichern
